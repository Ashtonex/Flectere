...
function buildStages() {
  ...
  for (let i = 0; i < COLS; i++) {
    for (let j = 0; j < ROWS; j++) {
      ...
      // Sense: loose, semi-random cloud (still listening, no structure yet)
      ...
      // Shape: clean organized flat grid
      shape[idx * 3] = x;
      shape[idx * 3 + 1] = y;
      shape[idx * 3 + 2] = Math.sin(u * Math.PI) * 0.25;

      // Shift: the grid twists into flowing motion
      ...
      // Scale: expanded, domed outward (growth)
  ...
  return { stages: [sense, shape, shift, scale], segments };
}
...
function MorphingLattice({
  ...
  useFrame((state) => {
    const t = Math.min(0.999, Math.max(0, progressRef.current)) * (stages.length - 1);
    ...
    for (let i = 0; i < COUNT; i++) {
      const x = lerp(from[i * 3], to[i * 3], local);
      const y = lerp(from[i * 3 + 1], to[i * 3 + 1], local);
      const z = lerp(from[i * 3 + 2], to[i * 3 + 2], local) + Math.sin(time * 0.4 + x) * 0.06;
    ...
    if (groupRef.current) {
      groupRef.current.rotation.y = Math.sin(time * 0.08) * 0.25 + progressRef.current * 0.6;
  ...
  return (
    <group ref={groupRef}>
      <points>
        ...
        <pointsMaterial size={0.06} color="#E8D4A0" transparent opacity={0.95} sizeAttenuation depthWrite={false} />
      </points>
      <lineSegments>
        ...
        <lineBasicMaterial color="#9BA1A8" transparent opacity={0.35} depthWrite={false} />
      </lineSegments>
    </group>
  );
}
